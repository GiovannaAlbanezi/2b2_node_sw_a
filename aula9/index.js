...
function withdraw(){
    ...
    .prompt([
        ...
        const accountNamec= answer['accountName']
        ...
        .prompt([
            {
                name:'amount',
                message:'Qualo valor da retirada?',
            }
        ])
    }).then((answer) => {
        const amount = answer ['amount']
...
function removeAmount(accountName, amount){
    ...
    if(!amount){
        console.log(
            chalk.bgRad.Black('Occoreu um erro, tente novamente mais tarde')
        )
            return withdraw()
        }
        ...
        accountData.balance = parseFloat(account.balance) - parseFloat(amount)
        fs.writeFileSync(
            `accounts/${accountName}.json`,
            JSON.stringify(accountData),
            function (err){
        ...
        console.log(
            chalk.green(`Foi sacado: R$ ${accoount} da conta. Saldo: R$ ${accountData.balance}.`)
        )
}